			...
			<div class="space-y-6">
				...
				<div>
					...
					<input
						...
						:class="{ 'border-red-500 focus:border-red-500 focus:ring-red-500/20': filenameError }"
					/>
				...
				<div>
					...
					<div class="flex gap-2">
						...
						>
							<svg v-if="!isSelectingDirectory" class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
			...
			<div class="space-y-6">
				...
				<div>
					...
					<div class="space-y-3">
						<div
							...
							:class="quality === qualityOption.value
								? 'bg-blue-600/20 border-blue-500 text-blue-300'
								: 'bg-zinc-800/40 border-zinc-600/60 hover:border-zinc-500/80 text-gray-300 hover:text-white'"
						>
							<div class="flex items-center justify-between">
								<div>
									<div class="font-medium text-sm">{{ qualityOption.label }}</div>
									<div class="text-xs opacity-70">{{ qualityOption.description }}</div>
								</div>
								<div class="flex items-center gap-2">
									<div class="text-xs text-gray-400">{{ qualityOption.fileSize }}</div>
									<div
										class="w-2 h-2 rounded-full"
										:class="quality === qualityOption.value ? 'bg-blue-500' : 'bg-zinc-600'"
									></div>
								</div>
							</div>
						</div>
				...
				<div class="p-4 bg-zinc-800/40 rounded-lg border border-zinc-600/60">
					<div class="flex items-center gap-2 mb-3">
						...
						<span class="text-sm font-medium text-gray-300">Export Summary</span>
					</div>
					<div class="space-y-2 text-sm">
						<div class="flex justify-between">
							<span class="text-gray-400">File:</span>
							<span class="text-white font-mono text-xs">{{ filename || 'unnamed' }}.{{ format }}</span>
						</div>
						<div class="flex justify-between">
						...
						<div class="flex justify-between">
							...
							<span class="text-white">{{ quality.charAt(0).toUpperCase() + quality.slice(1) }}</span>
						</div>
					</div>
				</div>
			</div>
...
const setDefaultDirectory = async () => {
	try {
		...
		try {
			const downloadsDir = await window.electron.ipcRenderer.invoke("GET_PATH", "downloads")
			if (downloadsDir) {
