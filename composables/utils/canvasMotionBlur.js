...
export class CanvasFastBlur {
    ...
    initCanvas(canvas) {
        if (!canvas) {
            console.warn('[CanvasFastBlur] No canvas provided');
            return false;
        }
        ...
        if (!this.ctx) {
            console.warn('[CanvasFastBlur] Failed to get 2d context');
            return false;
        }
    ...
    gBlur(blur = null) {
        ...
        ctx.globalAlpha = 1;
        console.log(`[CanvasFastBlur] Gaussian blur applied in ${Date.now() - start}ms`);
        return true;
    }
