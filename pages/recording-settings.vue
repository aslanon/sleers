<template>
	<div class="min-h-screen bg-[#1e1e1e]">
		...
		<div
			class="h-12 bg-[#2d2d2d] flex items-center justify-center border-b border-[#404040] drag-region"
		>
		...
		<div class="flex">
			<!-- macOS Style Sidebar -->
			<div
				class="w-52 bg-[#252525] border-r border-[#404040] min-h-[calc(100vh-48px)]"
			>
				<div class="py-4">
					<button
						v-for="tab in tabs"
						...
						:class="
							activeTab === tab.id ? 'bg-[#007aff] text-white' : 'text-gray-300'
						"
					>
						...
						<div class="space-y-8">
							<div class="grid grid-cols-2 gap-8">
								<div>
									...
									>
									<select
										v-model="videoSettings.quality"
										class="w-full bg-[#2d2d2d] border border-[#404040] rounded-md px-3 py-2 text-white focus:outline-none focus:ring-2 focus:ring-[#007aff] focus:border-[#007aff]"
									>
								...
								<div>
									...
									>
									<select
										v-model="videoSettings.frameRate"
										class="w-full bg-[#2d2d2d] border border-[#404040] rounded-md px-3 py-2 text-white focus:outline-none focus:ring-2 focus:ring-[#007aff] focus:border-[#007aff]"
									>
							...
							<div class="space-y-4">
								<div class="flex items-center">
									...
									/>
									<label for="capture-cursor" class="ml-3 text-sm text-white"
										>Show cursor in recording</label
									>
								</div>
							</div>
						</div>
						...
						<div class="space-y-6">
							<div class="flex items-center">
								<input
									...
									class="w-4 h-4 text-[#007aff] bg-[#2d2d2d] border-[#404040] rounded focus:ring-[#007aff]"
								/>
								<label for="system-audio" class="ml-3 text-sm text-white"
									>Include system audio</label
								>
							</div>
							...
							<div class="flex items-center">
								<input
									...
									class="w-4 h-4 text-[#007aff] bg-[#2d2d2d] border-[#404040] rounded focus:ring-[#007aff]"
								/>
								<label for="microphone" class="ml-3 text-sm text-white"
					...
					<div v-if="activeTab === 'general'">
						...
						<div class="space-y-8">
							<div class="grid grid-cols-2 gap-8">
								<div>
									<label class="block text-sm font-medium text-white mb-3"
										>Recording Delay</label
									>
									<select
										v-model="generalSettings.delay"
										class="w-full bg-[#2d2d2d] border border-[#404040] rounded-md px-3 py-2 text-white focus:outline-none focus:ring-2 focus:ring-[#007aff] focus:border-[#007aff]"
									>
								...
								<div>
									...
									>
									<select
										v-model="generalSettings.exportFormat"
										class="w-full bg-[#2d2d2d] border border-[#404040] rounded-md px-3 py-2 text-white focus:outline-none focus:ring-2 focus:ring-[#007aff] focus:border-[#007aff]"
									>
							...
							<div>
								...
								<div class="flex gap-2">
									...
									<button
										...
										class="px-4 py-2 bg-[#007aff] hover:bg-[#0056b3] rounded-md text-white text-sm"
									>
										Browse
									</button>
								</div>
		...
		if (window.electron?.ipcRenderer) {
			...
			if (savedSettings) {
				if (savedSettings.video)
					Object.assign(videoSettings.value, savedSettings.video);
				if (savedSettings.audio)
					Object.assign(audioSettings.value, savedSettings.audio);
				if (savedSettings.general)
					Object.assign(generalSettings.value, savedSettings.general);
			}
		}
	} catch (error) {
